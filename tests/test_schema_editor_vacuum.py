...
def test_schema_editor_vacuum_not_in_transaction(fake_model):
    ...
    with pytest.raises(SuspiciousOperation):
        schema_editor.vacuum(fake_model._meta.db_table)
...
def test_schema_editor_vacuum(fake_model, kwargs, query):
    schema_editor = PostgresSchemaEditor(connection)

    with CaptureQueriesContext(connection) as ctx:
        schema_editor.vacuum(fake_model._meta.db_table, **kwargs)
...
@pytest.mark.django_db(transaction=True)
def test_schema_editor_vacuum_columns(fake_model):
    schema_editor = PostgresSchemaEditor(connection)

    with CaptureQueriesContext(connection) as ctx:
        schema_editor.vacuum(
